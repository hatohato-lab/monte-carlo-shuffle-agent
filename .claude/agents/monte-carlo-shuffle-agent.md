...
## 任務
リストを**ランダムに並べ替える**純粋関数 `sort` … ではなく `shuffle(xs)` を `candidate.py` に実装する。
引数は壊さず、新しいリストを返す。乱数は `random` モジュールを使う（オラクルが種を固定して再現する）。

## 合否（オラクルが決める・モンテカルロ）
外部オラクル `eval/oracle.py` が、固定種で N 回実行し次を確認する:

- 妥当性: 毎回、入力の並べ替え（同じ要素・同じ個数）になっている。
- 一様性: 各位置に各要素がほぼ均等に来る（位置ごとのカイ二乗が閾値未満）。
